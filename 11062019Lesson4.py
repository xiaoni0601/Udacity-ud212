...
def best_hand(hand):
    ...
    return max(itertools.combinations(hand, 5), key=hand_rank)##(hand, 5)意思是7选5进行组合，key约束选择hand_rank.
    # Your code here
    # 为高效循环而创建迭代器的函数
    # https://docs.python.org/zh-cn/3/library/itertools.html
    # combinations()：for example: combinations('ABCD', 2) OUTPUT：AB AC AD BC BD CD
